...
pair<bool, Move> makeMove(Controller &controller) {
    ...
    // 3️⃣ No safe move — minimize opponent scoring opportunities
    int bestIndex = 0;
    int minOpponentCaptures = 1000000000; // large number
    int oppCaptures, oppCount;
    const Move *oppMvPtr;

    Board tempBoard; // allocate once and reuse

    for (i = 0; i < totalMoves; ++i) {
        mvPtr = &validMoves[i];
        tempBoard = board; // reuse existing temp board
        tempBoard.makeMove(*mvPtr, controller.getMySide());  // simulate our move


        const auto oppMoves = tempBoard.getValidMoves();
        oppCount = oppMoves.size();
        oppCaptures = 0;

        for (j = 0; j < oppCount; ++j) {
            oppMvPtr = &oppMoves[j];
            if (tempBoard.isCapturingMove(*oppMvPtr))
                ++oppCaptures;
        }

        if (oppCaptures < minOpponentCaptures) {
            minOpponentCaptures = oppCaptures;
            bestIndex = i;
        }
    }
    ...
    Log("Minimizing opponent score move: " + to_string(bestMove.row) + " " + to_string(bestMove.col)
        + " (Opponent captures after: " + to_string(minOpponentCaptures) + ")");
